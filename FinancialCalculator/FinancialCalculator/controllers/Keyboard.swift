...
//  FinancialCalculator
//
//  Created by Shenali Samaranayake on 3/2/20.
...
class Keyboard: UIView {
    ...
    // the button functionality for all the buttons in the keyboard
    @IBAction func onButtonPressed(_ sender: UIButton) {
        let cursorPosition = getCursorPosition()
               
               if let currentText = self.activeTextField.text {
                   switch KeyboardButton(rawValue: sender.tag)!{ //special characteristics functionality
                   case .period:
